...
{
    xTimerStop( blehr_tx_timer, 1000 );
}
...
blehr_tx_hrate_reset(void)
{
    ...
    if (xTimerReset(blehr_tx_timer, 1000 ) == pdPASS) {
        rc = 0;
    } else {
        rc = 1;
    }
...
/* Temperature task - runs every 200ms to simulate temperature reading and notify */
static void
blehr_temperature_task(void *pvParameters)
{
    int rc;
    struct os_mbuf *om;
    ...
    while (1) {
        ...
        if (!temp_notify_state) {
            continue;  /* Skip notification if not subscribed */
        }

        /* Simulate temperature variation: 36.50°C to 36.60°C */
        temp_value = 3650 + (rand() % 11);  /* Value in 0.01°C units */

        /* Build temperature notification in Health Thermometer format:
         * Byte 0: Flags (0x00 = Celsius, 0.1°C resolution, no timestamp)
         * Bytes 1-4: Temperature in IEEE 11073-20601 format (32-bit float)
         * 
         * IEEE 11073 format: mantissa (24-bit) + exponent (8-bit)
         * For 0.01°C resolution: exponent = -2 (0xFE as signed), mantissa = temp_value
         */
        temp_data[0] = 0x00;  /* Flags: Celsius, no timestamp */
        temp_data[1] = (temp_value >> 0) & 0xFF;  /* Mantissa byte 0 */
        temp_data[2] = (temp_value >> 8) & 0xFF;  /* Mantissa byte 1 */
        temp_data[3] = (temp_value >> 16) & 0xFF; /* Mantissa byte 2 */
        temp_data[4] = 0xFE;  /* Exponent: -2 (meaning value * 10^-2 = °C) */

        /* Send temperature notification */
        om = ble_hs_mbuf_from_flat(temp_data, sizeof(temp_data));
        rc = ble_gatts_notify_custom(conn_handle, temp_meas_handle, om);

        if (rc != 0) {
            MODLOG_DFLT(ERROR, "Failed to notify temperature; rc=%d\n", rc);
        }
    }
}
